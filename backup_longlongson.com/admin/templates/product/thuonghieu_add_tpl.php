<?php
	$mstt=get_stt_type_level('product_thuonghieu',$type,$level);
	...
	function get_level($i,$id,$idl)
	{
		...
		$d->reset();
		$sql="select id,ten_vi as ten from #_product_thuonghieu where level='$i' and type='".$_GET['type']."' $ssql order by stt,id desc";
		$d->query($sql);
		$res=$d->result_array();
	...
	<div class="row">
        <div class="col-md-12">
        	<div class="box box-info">
                <div class="box-body">
                <form name="frm" method="post" action="index.php?com=<?=$_GET['com']?>&act=save_thuonghieu&type=<?=$type?>" enctype="multipart/form-data" class="form-horizontal">
					...
					<?php if($config['type'][$type]['photo_thuonghieu']==true || $config['type'][$type]['thumb_thuonghieu']==true){?>
					<div class="form-group">
					...
					<div class="form-group">
                		...
                		<div class="col-sm-10">
							...
							<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=<?=$_GET['com']?>&act=man_thuonghieu&type=<?=$type?>&level=<?=$level?>'" class="btn btn-danger" />
	                    </div>
                	</div>
				</form>
                </div>
