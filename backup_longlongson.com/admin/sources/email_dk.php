...
if(isset($_POST["cb_item"])){
		...
		$mail->SetFrom('noreply@iambasix.vn','http://iambasix.vn/');        // SMTP account password

		//Thiet lap thong tin nguoi gui va email nguoi gui
		$mail->SetFrom('noreply@iambasix.vn','http://iambasix.vn/');
		...
		//Thiết lập tiêu đề
		$mail->Subject    = "[http://iambasix.vn/] - ".$_POST['tieude'];
		$mail->IsHTML(true);
		...
		if($data['file']){
			$mail->AddAttachment(_upload_tailieu.$data['file']);
		}
