...
<?php
    ...
    if($_POST["id_danhmuc"]==0){
    	...
    	$str_van = "select * from table_product where $moi!=0 and type='product' and id_list='".$_POST["id_danhmuc"]."' and hienthi=1 order by stt asc,id desc";
    }
...
<div class="tt-product-item">
  <div class="tt-product">
    <div class="tt-product-img">
      <a href="san-pham/<?=$row['tenkhongdau']?>.html" title="<?=$row['ten_'.$lang]?>">
        <img src="resize/280x225/2/<?=_upload_product_l.$row['photo']?>" alt="<?=$row['ten_'.$lang]?>">
      </a>
    </div>
    <div class="tt-product-title">
      <h4><a href="san-pham/<?=$row['tenkhongdau']?>.html" title="<?=$row['ten_'.$lang]?>">
        <?=$row['ten_'.$lang]?>
      </a></h4>
      <p>
        Hãng sản xuất: <span><?=get_thuonghieu($row['id_thuonghieu'])?></span>
      </p>
      <p class="itemprice">
        Giá: <span class="price"><?=($row['giaban']!=0) ? number_format($row['giaban'],0, ',', '.').' <sup>đ</sup>' : 'Liên hệ' ?></span>
      </p>
    </div>
  </div>
</div>
