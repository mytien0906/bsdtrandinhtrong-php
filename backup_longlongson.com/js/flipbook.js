...
window.pages = ['cover', 'cover', 'table-of-content', 'table-of-content', 'presentation', 'presentation', 'platforms', 'platforms', 
				'features', 'features', 'responsive-design', 'responsive-design', 'configuration', 'configuration', 'media', 'media',
				'back-cover', 'back-cover'];
window.tocIndex = 3; /* table of content index */
window.zoomStrength = 2; /* zoom strength */
window.slideShowInterval = 2000; /* slide show delay in miliseconds */
...
function hashChange(flipbook, fbCont) {
	...
	if(page == 1){
		...
		fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').animate( {
			opacity: 0,
...
function hideShadows(caller, arrow, flipbook, fbCont, posLeft) {
	...
	if(animate){
		...
		fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').stop().clearQueue();
	} 
...
function resizeFB(fbWidth, fbHeight, flipbook, fbCont) {
	...
	fbCont.find('http://www.pharmapatika.hu/ok/js/div.preview, div.next').css('top', (flipbook.find('div.turn-page-wrapper.first').height() - 86) / 2);
}
...
function centerBook(page, flipbook, fbCont, activeArrow) {
	...
	if ((page==1 || page == flipbook.data().totalPages) && !rotated()) {
		...
		if(page==1) 
			left = Math.floor(($(window).width() - pageWidth) * 0.5) - pageWidth;
		else
			left = Math.floor(($(window).width() - pageWidth) * 0.5);
		...
		if(parseInt(flipbook.css('left')) != left){
			...
			if(page == 1)
				hideShadows('center', activeArrow, flipbook, fbCont , Math.floor(($(window).width() - pageWidth) * 0.5) - pageWidth );
			else 
				hideShadows('center', activeArrow, flipbook, fbCont , Math.floor(($(window).width() - pageWidth) * 0.5));
		}
		
	} else {
			flipbook.animate({left: Math.floor(($(window).width() - width) * 0.5) }, options);
			hideShadows('center', activeArrow, flipbook, fbCont , Math.floor(($(window).width() - width) * 0.5));
	}	
}
...
function fbFirstRun(flipbook, fbCont) {
	...
	fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').stop().clearQueue();
	fbCont.find('div.preview').stop().clearQueue();
	fbCont.find('div.preview').css('left', $(window).width() * 0.5);
	fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').css('left', $(window).width() * 0.5);
	// adjust shadows
	hideShadows('turned', 'false', flipbook, fbCont, 'first run');		
	
	// show bottom ui
	if($('div.fb-nav').hasClass('small')){
		$('div.fb-nav').animate( {
			opacity: 1,
			top: '-55'
		}, 1000, 'easeOutExpo');
...
jQuery(document).ready(function($) {
	...
	flipbook.bind('turned', function(e, page) {	
		...
		if(slideshow) {
			flipbook.delay(slideshowDelay).queue(function() {
				$this.turn('next');
	...
	hash = checkHash();
	if(hash != 1 && window.pages.length < 2) {
		flipbook.turn({
	...
	flipbook.find('div.turn-page-wrapper').dblclick(function(e) { 
		var zoomFactor = window.zoomStrength,
			zoomCont,
			fbTopMargin,
			fbLeftMargin,
			fbZoomedBorder,
		...
		fbTopMargin = parseInt(flipbook.css('margin-top'));
		fbLeftMargin = parseInt(flipbook.css('left'));
		
		// set the scroll at the begening of zoom so there is no jump when you move your mouse
		if(e.pageY == undefined)
			e.pageY = fbTopMargin;
		if(e.pageX == undefined)
			e.pageX = fbLeftMargin;
		
		zoomCont.children().css('margin-top', fbTopMargin - e.pageY);
		zoomCont.children().css('margin-left', fbLeftMargin - e.pageX);
		...
		if(!largeImage) {
			zoomCont.find('img.bg-img').css({
				...
				'margin-left': fbLeftMargin - e.pageX,
				'opacity': 1
			});
		...
		zoomCont.find('.enlarge').each(function() {
			...
			$this.css({
				'font-size': (parseInt($this.css('font-size')) * zoomFactor) + "px" + " !important",
				'line-height': (parseInt($this.css('line-height')) * zoomFactor) + "px" + " !important"
			});
		});
		...
		fbCont.find('div.zoomed.double.odd img.bg-img').css('margin-left', - (zoomCont.width()));
		...
		zoomCont.css('left', ($(window).width() - zoomCont.width()) / 2);
		...
		function closeZoom() {
			...
			zoomCont.animate( { opacity: 0 }, 300, function(){
				$(this).remove();
				zoomed = false;
				fbNav.find('http://www.pharmapatika.hu/ok/js/ul li.zoom').trigger('mouseout');
			});
		...
		if(touch) {
			zoomCont.on('touchmove', function(evt){
				...
				if(fbLeftMargin - t[0].pageX >  - zoomCont.children().width() + zoomCont.width()) {
					zoomCont.find(' > div').css('margin-left', fbLeftMargin - t[0].pageX);
					zoomCont.find('img.bg-img').css('margin-left', fbLeftMargin - t[0].pageX);
				} else {
					zoomCont.find('img.bg-img').css('margin-left', - zoomCont.children().width() + zoomCont.width());
					zoomCont.find(' > div').css('margin-left', - zoomCont.children().width() + zoomCont.width());
				} 
			});
		}
		...
		zoomCont.mousemove(function(evt){
			if(fbTopMargin - evt.pageY >  - zoomCont.children().height() + zoomCont.height()) {
				...
				zoomCont.find('img.bg-img').css('margin-top', fbTopMargin-evt.pageY);
			} else {
				zoomCont.find('img.bg-img').css('margin-top', - zoomCont.children().height() + zoomCont.height());
				zoomCont.find(' > div').css('margin-top', - zoomCont.children().height() + zoomCont.height());
			} 

			if(fbLeftMargin - evt.pageX >  - zoomCont.children().width() + zoomCont.width()) {
				zoomCont.find(' > div').css('margin-left', fbLeftMargin-evt.pageX);
				zoomCont.find('img.bg-img').css('margin-left', fbLeftMargin-evt.pageX);
			} else {
				zoomCont.find('img.bg-img').css('margin-left', - zoomCont.children().width() + zoomCont.width());
				zoomCont.find(' > div').css('margin-left', - zoomCont.children().width() + zoomCont.width());
			} 
	...
	var tocIndex = window.tocIndex,
		slideshowDelay = window.slideShowInterval
		navWidth = 0,
		i = 0;
	...
	// Table Of Content
	fbNav.find('http://www.pharmapatika.hu/ok/js/ul li.toc').on('click', function(e){
	...
	// Zoom
	fbNav.find('http://www.pharmapatika.hu/ok/js/ul li.zoom').on('click', function(e){
		flipbook.find('div.turn-page-wrapper').each(function() {
			var $this = $(this);
	...
	fbNav.find('ul li.show-all').on('click', function(e){
		...
		fbCont.find('http://www.pharmapatika.hu/ok/js/div.preview, div.next').animate( { opacity: 0 }, 300);
		...
		paddingHorizontal = parseInt(showAll.find('http://www.pharmapatika.hu/ok/js/div.show-all-thumb.odd').css('margin-right'));
		...
		borderSize = parseInt(showAll.find('http://www.pharmapatika.hu/ok/js/div.show-all-thumb.odd').css('border-right-width'));
		...
		showAll.find('div.show-all-thumb').each(function(){
			...
			$this.find('.enlarge').each(function(){
				$(this).css('font-size', Math.round(parseInt($(this).css('font-size')) * percentage) + 'px !important');
				$(this).css('line-height', Math.round(parseInt($(this).css('line-height')) * percentage) + 'px !important');
			});
		...
		fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').animate( { opacity: 0 }, 300);
		...
		showAll.css({
			'left': ($(window).width() - showAll.width()) / 2 + 'px'
		});
		...
		showAll.find('div.show-all-thumb').on('click', function(){
			...
			fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').css( { opacity: 0 } );*/
	...
	/* Next & Previous */
	fbCont.find('http://www.pharmapatika.hu/ok/js/div.next').on('click', function(e){
		if(pageTurning)
			return;
		activeArrow = 'right';	
		flipbook.trigger('mouseover');
	...
	$(window).on('resize', function() {
		...
		if ((currentID == 1 || currentID == flipbook.data().totalPages)) {
			...
			if(currentID == 1) 
				left = Math.floor(($(window).width() - flipbook.width()/2) * 0.5) - flipbook.width()/2;
			else
				left = Math.floor(($(window).width() - flipbook.width()/2) * 0.5);
	
			if(parseInt(flipbook.css('left')) != left)
				flipbook.css({'left': left });
			
		} else {
				flipbook.css( {'left': Math.floor(($(window).width() - flipbook.width()) * 0.5) });
		}	
		...
		var $this = $(this),
			width = $this.width(),
			height = $this.height(),
			fbNext = fbCont.find('http://www.pharmapatika.hu/ok/js/div.next'),
	...
	$(window).bind('keydown', function(e) { // keyboard events
		if (e.keyCode == 37) {
			...
			fbCont.find('div.zoomed').animate( { opacity: 0 }, 300, function(){
				$(this).remove();
				zoomed = false;
				fbNav.find('http://www.pharmapatika.hu/ok/js/ul li.zoom').mouseout();
