...
function get_items(){
	...
	if(isset($_REQUEST['id_tv']) && (int)$_REQUEST['id_tv']>0)
	{
		$sql.=" and id_tv=".(int)$_REQUEST['id_tv'];	
	}

	if((int)$_REQUEST['tinhtrang']>0)
	{
		$sql.=" and trangthai=".(int)$_REQUEST['tinhtrang'];	
	}

	if($_REQUEST['ngaytao']!='')
	{
		$ngay=strtotime($_REQUEST['ngaytao']);
		$ngayke=$ngay+86399;
		$sql.=" and ( ngaytao between $ngay and $ngayke)"; 	
	}
