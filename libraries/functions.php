...
function getPriceBDS($giachu,$donvi,$giatinh){
	global $config;
	switch ($giatinh) {
		case 1:
			return $giachu.' '.$donvi;
			break;
		case 2:
			return $giachu.' '.$donvi.'/'.$config['dangtin']['giatinh'][$giatinh];
			break;
		case 3:
			return $giachu.' '.$donvi.'/'.$config['dangtin']['giatinh'][$giatinh];
			break;
...
function encrypt_password($str,$salt){return md5('$choquynhon@'.$str.$salt);}
...
function getRows($pid,$table,$data){
	global $d,$lang;
	$ex_array = implode(',',$data);
	$sql = "select $ex_array from #_$table where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row;
}
